...
public:
  Selection(std::filesystem::path base, std::vector<std::string> ships)
      : base(std::move(base)), ships(std::move(ships)) {}
  ...
  [[nodiscard]] size_t getChoice() const { return choice; }
  [[nodiscard]] size_t getNum() const { return ships.size(); }
