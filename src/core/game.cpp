...
void Game::update(Stage stage) {
  if (current == WorldType::begin) {
    const auto &selector = dynamic_cast<model::Selection &>(
        *worlds[current].findModel(id)->second);
    if (selector.isConfirmed()) {
      current = WorldType::main;

      parser::loadAndAddPlayer(selector.getPath(), worlds[current]);
      parser::loadAndAddShield("res/shields/shield-config.json",
                               worlds[current]);
      parser::loadAndAddLevel("res/levels/level" + std::to_string(level++) +
                                  ".json",
                              worlds[current]);
  ...
  if (current == WorldType::main) {
    ...
    if (stage == Stage::victory) {
      ...
      if (not parser::loadAndAddLevel(path, worlds[current]))
        done = true;
    }
    if (stage == Stage::defeat or done) {
      current = WorldType::end;
      const auto score = worlds[WorldType::main].getScore();
      parser::loadAndAddLeaderBoard("res/data/leaderboard.json",
                                    worlds[current], score);
    }
  }
}
...
int Game::start() {
  ...
  duration<double> previousTime =
      util::Stopwatch::get().time().time_since_epoch();
  ...
  while (shouldRun) {
    ...
    if (frameCounter >= 1s) {
      std::cout << static_cast<double>(frames) /
                       duration_cast<seconds>(frameCounter).count()
                << " fps\n";
      frameCounter -= 1s;
      frames = 0;
    }
