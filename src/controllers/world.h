...
public:
  ...
  void erase(size_t id) { controllers.erase(id); }

  void clear() { controllers.clear(); }

private:
  // this needs to be a multimap to allow multiple controllers, controlling the
  // same model
  std::unordered_multimap<size_t, std::shared_ptr<controller::Abstract>>
      controllers;
};
