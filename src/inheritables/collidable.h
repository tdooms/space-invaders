...
enum class Wall { right, left, bottom, top };
...
struct CollideData {
  CollideData(Vec2d pos, Vec2d vel, Vec2d dim, double rotation, double damage,
              double mass, model::Type type, model::Side side)
      : pos(pos), vel(vel), dim(dim), rotation(rotation), damage(damage),
        mass(mass), type(type), side(side) {}
