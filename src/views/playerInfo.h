...
public:
  explicit PlayerInfo(const std::shared_ptr<model::Abstract> &model)
      : Abstract(model) {
  ...
  void receive(Event type) override {
    ...
    if (cooldown.currentDuration().count() != 0) {
      length =
          cooldownLength * cooldown.msRemaining() / cooldown.currentDuration();
    }

    const auto backLiveSize =
        transform.scale(model.getMaxLives() / 3.0, height);
