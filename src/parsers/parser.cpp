...
nlohmann::json openJson(const std::filesystem::path &path) {
  if (not std::filesystem::exists(path)) {
    throw std::runtime_error("file with path: " + path.string() +
                             " does not exist");
  }
...
bool loadAndAddPlayer(const std::filesystem::path &path, core::World &world) {
  try {
    ...
    const auto id = world.addEntity<entities::Player>(std::forward_as_tuple(
        start, velocity, dim, speed, lives, color, texture, bulletInfo));
...
bool loadAndAddShield(const std::filesystem::path &path, core::World &world) {
  try {
    ...
    for (const auto &elem : json["shields"]) {
      ...
      world.addEntity<entities::Shield>(
          std::forward_as_tuple(posititon, dimensions, lives, color, texture));
    }
  } catch (std::exception &e) {
    return false;
  }
...
bool loadAndAddPreview(const nlohmann::json &json, core::World &world,
  ...
  try {
    ...
    world.addEntity<entities::Enemy>(std::forward_as_tuple(
        start, velocity, dim, speed, lives, color, texture, bulletInfo));
  } catch (std::exception &e) {
    return false;
  }
...
size_t loadAndAddSelection(const std::filesystem::path &path,
  ...
  for (const auto &elem : std::filesystem::directory_iterator(path.string())) {
    if (elem.path().filename().string()[0] == '.')
      continue;
  ...
  const auto id = world.addObject<objects::Selection>(
      std::forward_as_tuple(path, std::move(names)));
  ...
  for (const auto &choice : choices) {
    ...
    world.addObject<objects::Text>(
        std::forward_as_tuple(choice["info"]["name"], currPos, 20));
    world.addObject<objects::Text>(std::forward_as_tuple(
        choice["info"]["description"], currPos + Vec2d(0, 0.3), 11));
    currPos.x += offset;
  }
...
bool loadAndAddLevel(const std::filesystem::path &path, core::World &world) {
  try {
    ...
    const auto enemyJson = openJson(
        "res/enemies/" + json["level"]["type"].get<std::string>() + ".json");

    const auto velocity = Vec2d(enemyJson["stats"]["horizontalSpeed"],
                                enemyJson["stats"]["downSpeed"]);
    const auto dim =
        Vec2d(enemyJson["stats"]["size"], enemyJson["stats"]["size"]);
    ...
    for (size_t i = 0; i < numEnemies; i++) {
      world.addEntity<entities::Enemy>(std::forward_as_tuple(
          curr, velocity, dim, speed, lives, color, texture, bulletInfo));
...
bool loadAndAddScore(const std::filesystem::path &path, core::World &world) {
  const auto json = openJson(path);
  world.addObject<objects::Score>(
      std::forward_as_tuple(Vec2d(json["x"], json["y"]), json["size"]));
  return true;
}
